...
const EditMovie = () => {
    ...
    // When we are accessing a link of an existing value, 
    // id will be a value of string
    if (id === undefined) {
        id = 0;
    }
    ...
    useEffect(() => {
        ...
        // Is this a new movie or an existing movie?
        if (id === 0) {
            // Adding a movie and making sure we clean up the form
            // if a user comes from the Edit movie form
            setMovie({
                id: 0,
                title: "",
                release_date: "",
    ...
    const handleSubmit = (event) => {
        ...
        if (movie.genres_array.length === 0) {
            Swal.fire({
                title: 'Error!',
                text: 'You must choose at leaset one genre!',
                icon: 'error',
        ...
        // Assume we are heading a new movie
        let method = "PUT";
        if (movie.id > 0) {
            method = "PATCH";
        }
    ...
    const handleCheck = (event, position) => {
        console.log("handle check called:", event.target.value, position)
